...
class FocalLoss(nn.Module):
    ...
    def forward(self, input, target):
        ...
        ce_loss = F.binary_cross_entropy_with_logits(p_pred, target, reduction="none")
        p_t = p_pred * target + (1 - p_pred) * (1 - target) # p_t = p if y = 1 else 1-p
        loss = ce_loss * ((1 - p_t) ** self.gamma) # log(p)(1 - p) ** gamma  if y = 1 else log(1-p)p ** gamma  
        
        if self.alpha >= 0:
            alpha_t = self.alpha * target + (1 - self.alpha) * (1 - target) # alpha if y = 1 else 1-alpha
            loss = alpha_t * loss

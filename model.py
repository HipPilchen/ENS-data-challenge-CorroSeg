...
"""Baseline CNN model for segmentation
"""
...
class baseline_CNN(nn.Module):
    def __init__(self):
        ...
        self.conv4 = nn.Conv2d(128, 256, kernel_size=3,
                               padding=(1, 1))  # Couche 4 : Convolution
        self.conv5 = nn.Conv2d(256, 512, kernel_size=3,
                               padding=(1, 1))  # Couche 5 : Convolution
    ...
    def forward(self, x):
        ...
        x = torch.sigmoid(self.fc2(x)).view(-1, 1, 36, 36)
        return x


"""Additive small UNet model for segmentation
"""
...
class SegmentationCNN(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.upconv4(x5)
        x = self.conv6(x + F.interpolate(x4, size=x.size()
                       [2:], mode='bilinear', align_corners=False))  # Skip connection

        x = self.upconv3(x)
        x = self.conv7(x + F.interpolate(x3, size=x.size()
                       [2:], mode='bilinear', align_corners=False))  # Skip connection

        x = self.upconv2(x)
        x = self.conv8(x + F.interpolate(x2, size=x.size()
                       [2:], mode='bilinear', align_corners=False))  # Skip connection

        x = self.upconv1(x)
        x = self.conv9(x + F.interpolate(x1, size=x.size()
                       [2:], mode='bilinear', align_corners=False))  # Skip connection
...
"""Concatenate small UNet model for segmentation
"""
...
class Jacard_UNet(nn.Module):
    def __init__(self):
        ...
        self.conv10 = nn.Conv2d(16, self.out_channels,
...
"""Additive CorroSeg UNet model for segmentation
"""
...
class UNet(nn.Module):
    ...
    def conv_block(self, in_channels, out_channels):
        block = nn.Sequential(
            ...
            nn.ConvTranspose2d(out_channels, out_channels, kernel_size=2,
                               stride=2, padding=0, output_padding=0)  # Adjusted
        )
        return block
    ...
    def forward(self, x):
        ...
        dec4 = F.interpolate(dec4, size=enc4.size()[
                             2:], mode='bilinear', align_corners=False)
        dec4 = dec4 + enc4  # Now add after resizing

        dec3 = self.decoder3(dec4)
        dec3 = F.interpolate(dec3, size=enc3.size()[
                             2:], mode='bilinear', align_corners=False)
        if self.dropout:
        ...
        dec2 = F.interpolate(dec2, size=enc2.size()[
                             2:], mode='bilinear', align_corners=False)
        if self.dropout:
        ...
        dec1 = F.interpolate(dec1, size=enc1.size()[
                             2:], mode='bilinear', align_corners=False)
        if self.dropout:
        ...
        out = F.interpolate(out, size=(36, 36),
    ...
    def unfreeze_blocks(self, num_blocks):
        encoder_blocks = [self.encoder1, self.encoder2,
...
"""Concatenation CorroSeg UNet model for segmentation
"""
...
class Cat_UNet(nn.Module):
    def __init__(self, pretrained=True, dropout=False, pdrop=None, batchnorm=True):
        ...
        self.upconv4 = nn.ConvTranspose2d(
            2048, 1024, kernel_size=2, stride=2, padding=0, output_padding=0)
        self.decoder4 = self.conv_block_short(2048, 1024, batchnorm=batchnorm)

        self.upconv3 = nn.ConvTranspose2d(
            1024, 512, kernel_size=2, stride=2, padding=0, output_padding=0)
        self.decoder3 = self.conv_block_short(1024, 512, batchnorm=batchnorm)

        self.upconv2 = nn.ConvTranspose2d(
            512, 256, kernel_size=2, stride=2, padding=0, output_padding=0)
        self.decoder2 = self.conv_block_short(512, 256, batchnorm=batchnorm)

        self.upconv1 = nn.ConvTranspose2d(
            256, 64, kernel_size=2, stride=2, padding=0, output_padding=0)
    ...
    def forward(self, x):
        ...
        up4 = F.interpolate(up4, size=enc4.size()[
                            2:], mode='bilinear', align_corners=False)
        ...
        up3 = F.interpolate(up3, size=enc3.size()[
                            2:], mode='bilinear', align_corners=False)
        ...
        up2 = F.interpolate(up2, size=enc2.size()[
                            2:], mode='bilinear', align_corners=False)
        ...
        up1 = F.interpolate(up1, size=enc1.size()[
                            2:], mode='bilinear', align_corners=False)
...
class EfficientNetBackbone(nn.Module):
    def __init__(self, backbone, fpn=False, selected_layers=None):
        ...
        self.selected_layers = selected_layers if selected_layers else [
            2, 4, 6, 8]
...
def get_backbone(backbone_name, fpn=False, pretrained=True):
    ...
    if backbone_name in backbone_factory:
        ...
        # Assuming a typical input size for ResNet
        dummy_input = torch.rand(1, 3, 36, 36)
        ...
        if isinstance(output_features, torch.Tensor):
            ...
            output_channels = sum([feature.size(1)
...
class BinarySegmentationModel(nn.Module):
    def __init__(self, fpn=False, backbone_name='resnet50', backbone_pretrained=True):
        ...
        self.backbone, output_channels = get_backbone(
            backbone_name, fpn=fpn, pretrained=backbone_pretrained)
        ...
        self.upsample = nn.Upsample(scale_factor=(
            18, 18), mode='bilinear', align_corners=False)
...
def get_model(model_name, backbone_name, backbone_pretrained=True, dropout=False, pdrop=None, batchnorm=True):
    """Enable to selec an architecture

    Args:
        model_name (str): name of the architecture
        backbone_name (str): name of the pretrained backbone used
        backbone_pretrained (bool, optional): Whether to use a pretrained backbone. Defaults to True.
        dropout (bool, optional): Whether to use dropout. Defaults to False.
        pdrop (_type_, optional): Intensity of the dropout. Defaults to None.
        batchnorm (bool, optional): Whether to use batchnormalization in the decoder. Defaults to True.
    ...
    if model_name == 'first_model':
        ...
        model = Cat_UNet(dropout=dropout, pdrop=pdrop,
                         batchnorm=batchnorm, pretrained=backbone_pretrained)
    return model

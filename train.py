...
def main(args):
    ...
    """Perform data augmentation and create dataloaders
    """      
    ...
    for epoch in tqdm(range(args.num_epochs)):
        ...
        for image, mask, well in tqdm(train_loader):
            ...
            train_iou += (-iou_score_class(outputs, mask)
                          ).item() * image.size(0)
        ...
        with torch.no_grad():
            for image, mask, well in tqdm(val_loader):
                ...
                # Apply threshold to get binary predictions
                preds = (outputs - args.threshold).round()
                val_iou += (-iou_score_class(outputs, mask)
                            ).item() * image.size(0)
        ...
        for image, _, _ in test_loader:  # Ignore the masks in the test loader
            ...
            if not args.random_walk:
                ...
                if torch.any(image < -100):
                    # Reset preds to zeros if conditions are met
                    preds = torch.zeros_like(preds).int()
                # Ensure consistent shape for all flattened masks
                # Explicitly specify the flattened shape
                flattened_mask = preds.cpu().numpy().reshape(-1, 36*36)
                ...
                if not np.any(preds_corrosion.cpu().numpy()) or not np.any(preds_background.cpu().numpy()):
                    ...
                    image = (image.cpu().numpy() - np.min(image.cpu().numpy())) * \
                        255/(np.max(image.cpu().numpy()) -
                             np.min(image.cpu().numpy()))
                    labels = random_walker(image, markers, beta=1, mode='bf')

                    labels = labels - 1
                predicted_masks.append(labels.flatten())
                
    """Create CSV file with predicted masks
    """
    predicted_masks = np.vstack(predicted_masks)

    df = pd.DataFrame(predicted_masks)
    # print(df)
    files = [f.replace('.npy', '')
             for f in os.listdir('data/processed/images_test')]
    df.index = files
...
def parser_args(parser):
    ...
    parser.add_argument('--model_need_GRAY', action="store_true",
                        help='Whether to tile in 3 channels or not, by default RGB 3 channels')
    parser.add_argument('--pretrained', action="store_true",
                        help="Whether to use a pretrained model or not")
    parser.add_argument('--scheduler', action="store_true",
                        help="Whether to use a scheduler or not")
    parser.add_argument('--dropout', action="store_true",
                        help="Whether to use a dropout or not")
    parser.add_argument('--p_dropout', type=float, default=None,
                        help="Probability of dropout if None standard value is used")
    parser.add_argument('--random_walk', action="store_true",
                        help="Whether to use a random walk or not")
